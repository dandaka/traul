...
describe("getThread / getThreadsByDate", () => {
  ...
  describe("getThread", () => {
    it("returns all messages for a thread_id ordered by sent_at", async () => {
      ...
      const results = await db.getThread("session-uuid-123");
    ...
    it("does not return messages from other threads", async () => {
      ...
      const results = await db.getThread("session-aaa");
      expect(results).toHaveLength(1);
      expect(results[0].content).toBe("Thread A");
    });

    it("returns empty array for non-existent thread", async () => {
      const results = await db.getThread("does-not-exist");
      expect(results).toHaveLength(0);
    });
  });

  describe("getThreadsByDate", () => {
    it("returns messages from threads within the date range", async () => {
      ...
      const results = await db.getThreadsByDate(dayStart, dayEnd);
      expect(results).toHaveLength(1);
      expect(results[0].content).toBe("Today's message");
    });

    it("excludes messages with null thread_id", async () => {
      const dayStart = 1700000000;
      const dayEnd = dayStart + 86400;
    ...
    it("returns empty array when no threads in range", async () => {
      const results = await db.getThreadsByDate(1700000000, 1700086400);
      expect(results).toHaveLength(0);
    });
  });
});
